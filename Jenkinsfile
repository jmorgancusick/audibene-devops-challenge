    ...
    stages {
        ...
        stage("develop-branch-deploy") {
            ...
            steps {
                ...
                sh 'helm upgrade --install --set image.id=$(docker inspect --format="{{index .RepoDigests 0}}" 482283577367.dkr.ecr.us-east-2.amazonaws.com/jmc/audibene-devops-challenge:latest) django-admin .'
            }
        }
    }
}